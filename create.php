...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    /* Validate ecole */
    $input_ecole = trim($_POST["taille"]);
    if(empty($input_ecole)){
        $ecole_err = "Veillez entrez une taille.";     
    } else{
        $ecole = $input_ecole;
    }
    
    /* Validate age */
    $input_age = trim($_POST["prix"]);
    if(empty($input_age)){
        $age_err = "Veillez entrez le prix.";     
    ...
    if(empty($name_err) && empty($ecole_err) && empty($age_err)){
        ...
        $sql = "INSERT INTO students (nom, taille, prix) VALUES (?, ?, ?)";
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="form-group">
                            <label>taille</label>
                            <textarea name="taille" class="form-control <?php echo (!empty($ecole_err)) ? 'is-invalid' : ''; ?>"><?php echo $ecole; ?></textarea>
                            <span class="invalid-feedback"><?php echo $ecole_err;?></span>
                        </div>
                        <div class="form-group">
                            <label>prix</label>
                            <input type="number" name="prix" class="form-control <?php echo (!empty($age_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $age; ?>">
                            <span class="invalid-feedback"><?php echo $age_err;?></span>
                        </div>
                        <input type="submit" class="btn btn-primary" value="Enregistrer">
                        <a href="indexx.php" class="btn btn-secondary ml-2">Annuler</a>
                    </form>
                </div>
            </div>        
        </div>
